...
struct msdisp_hal_funcs {
	s32 (*get_edid)(struct usb_device *udev,
			struct misctiming *misc_timing,
			u8 detail_count, u8 chip_id, u8 port_type,
			u8 sdram_type, u8 block, u8 *buf, u32 len);
	s32 (*get_hpd_status)(struct usb_device *udev, u32 *status);
	s32 (*set_video_in_info)(struct usb_device *udev, u16 width,
				 u16 height, u8 color, u8 byte_sel);
	s32 (*set_video_out_info)(struct usb_device *udev, u8 index, u8 color,
				  u16 width, u16 height);
	s32 (*trigger_frame)(struct usb_device *udev, u8 index, u8 delay);
	s32 (*set_trans_mode)(struct usb_device *udev, u8 mode, u8 *param,
			      u8 param_cnt);
	...
	s32 (*set_screen_enable)(struct usb_device *udev, u8 enable,
				 u8 chip_id, u8 port_type, u8 sdram_type);
	s32 (*event_proc)(struct usb_device *udev, struct usb_hal_event *event,
			  u8 chip_id, u8 port_type, u8 sdram_type);
	...
	s32 (*init_dev)(struct usb_device *udev, u8 chip_id, u8 port_type,
			u8 sdram_type);
};
...
const struct msdisp_hal_dev *msdisp_hal_find_dev(const struct usb_device_id *id,
